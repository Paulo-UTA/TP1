...
public class TP1 {
    ...
    public static void mostrarAlunos() {
        interC.showMsg("Os Alunos são: ");

        int i;
        for(i=0; i<=30; i++){
            if(nomeAlunos[0] == null){
                System.out.println("Não hã dados");
                break;
            }else{
                System.out.println("000"+(i+1)+"-"+ nomeAlunos[i]);
            }
        }
    ...
    private static void calcularMaxMinAvg() {
        interC.showMsg("As notas: ");

        int[] notasAluno = new int[MAX_ALUNOS];
        int media = 0;


        int notaMin = notasAlunos[0];
        int notaMax = notasAlunos[0];

        for (int i = 0; i < notasAlunos.length; i++) {
            media += notasAluno[i];
            float notaAvg = (float)media / notasAluno.length;
    ...
    public static void mostraResumo(){
        ...
        System.out.println("Nota Maxima é:" +notaMax);
        System.out.println("Nota Minima é:" +notaMin);
        System.out.println("Media é:" +notaAvg);
    }
    ...
    public static void mostrarTop() {
        ...
        interC.showMsg("As 3 melhores notas: ");

        int notasAlunos[] = new int[MAX_ALUNOS];
        int maiorValor1 = 0;
        int maiorValor2 = 0;
        int maiorValor3 = 0;

        for (int i = 0; i < notasAlunos.length; i++) {
            notasAlunos[i] = (int) ((Math.random() * MAX_ALUNOS));
        }

        for (int i = 0; i < notasAlunos.length; i++) {
            System.out.println(notasAlunos[i]);
            if (notasAlunos[i] > maiorValor1){
                maiorValor1 = notasAlunos[i];
            System.out.println("Maior valor 1: " + nomeAlunos[i] + "- " + maiorValor1);

        }else{
                if((notasAlunos[i] > maiorValor2) && (notasAlunos[i] != maiorValor1))
                    maiorValor2 = notasAlunos[i]  ;
                    System.out.println("Maior valor 2: " + nomeAlunos[i]+ maiorValor2);

            }
                if ((notasAlunos[i] > maiorValor3) && (notasAlunos[i] != maiorValor2)  && (notasAlunos[i] != maiorValor1))
                    maiorValor3 = notasAlunos[i]  ;
                    System.out.println("Maior valor 3: " + nomeAlunos[i]+maiorValor3);
    ...
    public static void mostrarPauta() {
        interC.showMsg("Pauta dos alunos");
        ...
        for(i=0; i<=30; i++){
            if(nomeAlunos[0] == null){
                ...
                System.out.println("000"+(i+1)+"-"+ nomeAlunos[i]+ "  -->  "+notasAlunos[i]);
            }
        }
    ...
    public static void pesquisar(String nome) {
        ...
        for (i=0;i<=MAX_ALUNOS;i++){
            if (nomeAlunos[i].toLowerCase().startsWith(nome.toLowerCase())){
                System.out.println("000"+(i+1)+" " +nomeAlunos[i]);
            }
        }
    ...
    */
    ...
    public static void pesquisar(int nota) {
        ...
        if (nomeAlunos[0]==null){
            ...
            for(int a=0; a<=notasAlunos.length -1; a++){
                if(nomeAlunos[a] != null && nota==notasAlunos[a]){
                    System.out.println(""+nomeAlunos[a] +""+notasAlunos[a]);
                }
            }
        }
